...
require_absolute_path "$SU_BIN" "su"
...
profiles="quick-fake quick-fake-ttl3 quick-fake-split quick-managed"
...
profile_count=0
{
  ...
  for profile in $profiles; do
    ...
    printf '{"id":"%s","provider":"nfqws-v1","provider_version":"%s","binary_digest":"sha256:%s","route_type":"zapret","ip_families":["ipv4"],"transports":["tcp"],"ports":[80,443],"queue":%s,"safety":"reviewed","strategy_digest":"sha256:%s","strategy":' "$profile" "$provider_version" "$binary_digest" "$MANAGED_QUEUE" "$strategy_digest"
    json_quote_file "$strategy_path"
    printf '}'
    profile_count=$((profile_count + 1))
  done
  printf '],"bundles":[{"id":"%s","category":"TSPU_RESTRICTED","required_domains":["%s"],"protocols":[{"transport":"tcp","port":80},{"transport":"tcp","port":443}],"ip_families":["ipv4"],"allowed_profiles":["quick-fake","quick-fake-ttl3","quick-fake-split","quick-managed"],"failure_route":"drop"}]}' "$bundle_id" "$domain"
} > "$catalog_tmp"
...
append_attempt() {
  ...
  printf '{"profile_id":"%s","target":"%s","protocol":"https","result":"%s","path_verified":%s,"cleanup_verified":%s,"route_evidence":"%s","nfqueue_packets":%s,"nfqueue_counter_delta":%s,"latency_ms":%s,"verification_duration_ms":%s,"http_status":%s' \
    "$profile" "$domain" "$result" "$path_ok" "$cleanup_ok" "$evidence" "$packets" "$counter_delta" "$latency_ms" "$duration_ms" "$http_status" >> "$attempts_file"
