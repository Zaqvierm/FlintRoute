...
> Соответствует `internal/probe/probe.go`, `internal/health/service.go`,
> `internal/artifact/artifact.go` на commit `4634515`.
> Файл переписан в UTF-8 (прежняя версия была в сломанной кодировке).

Этот документ фиксирует flowchart как основной алгоритмический контракт. Код
не обязан копировать каждый блок в отдельный файл. Наоборот: если каждый ромб
превратить в shell-скрипт, получится гнильная каша.
