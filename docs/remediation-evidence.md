...
| Exact-target dnsmasq observer confdir ownership | `tests/installer-lifecycle.sh` | PASS; UCI-provided `/etc/shadow` is rejected before it can enter the rollback manifest | local/mock |
