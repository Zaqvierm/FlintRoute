...
| Quick Zapret route/ownership evidence | `scripts/quick-zapret-check.sh`, `tests/zapret-quick-contract.sh` | Pending current-SHA Linux CI; each attempt now records a pre-request kernel route lookup, disables ambient HTTP proxies, snapshots routes/rules, and tags daemonized nfqws with a per-run ownership marker | Windows: NOT RUN LOCALLY; current-SHA Linux run required |
