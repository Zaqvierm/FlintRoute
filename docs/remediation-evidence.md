...
| Static installer-file ownership on uninstall | `install.sh`, `uninstall.sh`, `tests/installer-lifecycle.sh` | PASS (local fixture); install writes a root-owned, mode-0600 hash manifest for controller binary/helper/init/hotplug files; uninstall validates every present target before teardown and fences modified/foreign content | local/mock; Linux/OpenWrt runtime pending |
