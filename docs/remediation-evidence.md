...
| Exact-target dnsmasq observer confdir ownership | `tests/installer-lifecycle.sh`, `tests/dns-observer-bootstrap.sh` | PASS; installer and root bootstrap reject UCI/override `/etc/shadow` before it can enter the rollback manifest or be written | local/mock |
