...
> **Статус на `f177ca5`:** контракт и локальные safety-тесты актуальны. Все
...
8. безопасный TSPU fallback `Zapret → VLESS → DROP`. Smart DNS остаётся
   отдельным GEO-кандидатом и не подменяет DPI bypass.
