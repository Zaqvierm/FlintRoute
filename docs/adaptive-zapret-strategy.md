...
> **Статус на `e97f8dd`:** контракт и локальные safety-тесты актуальны. Все
...
8. безопасный TSPU набор кандидатов: Zapret, Smart DNS, VLESS и terminal DROP.
   Победитель определяется только по hard-filtered evidence; фиксированная
   последовательность не является политикой выбора. Smart DNS остаётся
   отдельным DNS-кандидатом и не подменяет DPI bypass.
...
Это не обещание фиксированного числа стратегий: в текущем pinned upstream нет
достоверного runtime-каталога «21 вариантов», поэтому FlintRoute не выдумывает
такое число. Результат любого режима считается только предложением кандидатов;
активация остаётся отдельным ChangeSet. Один worker сохраняется, потому что
upstream делит nft/NFQUEUE и process state. Режим, scan level, timeout, лог и
результат cleanup входят в evidence.

`blockcheck.sh` проверяет конкретный target через curl и не доказывает, что
любой браузерный/клиентский fingerprint прошёл через Zapret. Поэтому quick и
...
только с доменом, на котором есть evidence. Кандидат другого домена не
