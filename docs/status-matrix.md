...
| P13 | 50% | Harness, четыре маршрута, чистая установка и controlled reboot доказаны; расширенная fault/multi-client/soak матрица остаётся |
...
| P13.1 | частично | Direct, Zapret, VLESS и DROP; HTTPS с LAN-клиента; IPv6 skip записан явно | Smart DNS, UDP/QUIC, DNS TCP/UDP, полноценные IPv4/IPv6 cases |
...
| P13.3 | частично | Managed process checks, целый committed dataplane после controlled reboot, автоматический rollback при непроверенном management path | hard process termination по фазам, timer fault, power loss и повреждение state |
...
- Factory clean install, первая транзакционная активация и controlled reboot
  доказаны. Матрица аварийных завершений, срабатывание таймера при потере
  управления, несколько клиентов, понижение версии и удаление ещё требуют
  аппаратного прогона.
