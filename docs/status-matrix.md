...
| P12 | 85% | Ranking, scheduler, hysteresis, pin, quarantine и rollback state machine готовы; осталась аппаратная проверка |
...
| Bounded scheduler и переключение Zapret-профилей с cooldown/pin/quarantine | да | модульные, rollback и race | два bundles, два profiles и negative control |
...
| SHA-256 пакета OpenWrt, откат установки/обновления и проверенное удаление | да | shell-проверка жизненного цикла и обновление на Flint 2 | чистая установка, понижение версии и удаление |
...
- Матрица перезагрузок и аварийных завершений, срабатывание таймера при потере
  управления, несколько клиентов, чистая установка, понижение версии и удаление
  ещё требуют аппаратного прогона. Обновление активной headless-установки уже
  доказано.
