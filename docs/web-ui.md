...
## Доступ к UI на роутере

Если FlintRoute запущен на OpenWrt с обычным listener
`127.0.0.1:8787`, адрес `192.168.x.1:8787` с компьютера работать не будет:
`127.0.0.1` — loopback самого роутера, а не LAN-адрес клиента. Безопасный
способ открыть UI с компьютера — локальный SSH-forward:

```powershell
ssh -N -L 127.0.0.1:8787:127.0.0.1:8787 root@192.168.0.1
```

После запуска туннеля открывается `http://127.0.0.1:8787/` в браузере на
компьютере. Не переводите listener на `0.0.0.0` только ради удобства: пока
controller работает с root-привилегиями, loopback bind остаётся обязательной
...
ресурсы общие; выбор профиля и его применение остаются отдельной bounded
операцией, а не скрытым перебором.
